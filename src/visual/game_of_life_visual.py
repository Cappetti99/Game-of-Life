...
class GameOfLife:
    def __init__(self, width: int = 120, height: int = 80, cell_size: int = 10):
        ...
        # Grid size (in cells), and cell rendering size (in pixels)
        self.grid_width = width
        self.grid_height = height
        self.cell_size = cell_size

        # Fixed window size (we draw the grid into a separate surface and blit it)
        ...
        # Surface that holds the grid drawing (scaled by cell_size)
        self.grid_surface = pygame.Surface((width * cell_size, height * cell_size))

        # Offset for panning (currently always 0, but kept for future scroll/pan)
        self.offset_x = 0
        self.offset_y = 0

        # State grid: 0 dead, 1 alive
        ...
        self.running = False
        self.generation = 0
        self.speed = 10  # simulation fps when running
    ...
    def add_glider_gun(self, x: int, y: int):
        # Gosper glider gun pattern (hard-coded coordinates), needs a bit of space
        if x + 36 >= self.grid_width or y + 9 >= self.grid_height or x < 0 or y < 4:
            return

        # The assignments below “draw” the gun directly into the grid
    ...
    def step(self):
        # Neighbor count via 8 shifted grids (wrap-around via np.roll)
    ...
    def screen_to_grid(self, sx: int, sy: int) -> tuple[int, int]:
        # Convert window pixel coordinates to grid cell coordinates (considering offset)
        return (sx + self.offset_x) // self.cell_size, (sy + self.offset_y) // self.cell_size

    def set_cell(self, sx: int, sy: int, value: int):
        # Set a cell based on mouse position (ignore if outside grid)
        gx, gy = self.screen_to_grid(sx, sy)
        if 0 <= gx < self.grid_width and 0 <= gy < self.grid_height:
            self.grid[gy, gx] = value

    def draw(self):
        # Draw the grid into grid_surface (then we blit it to the main screen)
        self.grid_surface.fill(BLACK)

        # Draw grid lines only when cells are big enough (otherwise it becomes visual noise)
        if self.cell_size >= 4:
        ...
        # Draw live cells as filled rectangles
    ...
    def draw_ui(self):
        ...
        # Small help line when paused (so it doesn't distract while running)
        if not self.running:
    ...
    def handle_events(self) -> bool:
        # Centralized input handling: returns False when we want to quit
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    gx, gy = self.screen_to_grid(*pygame.mouse.get_pos())
                    # +4 because the gun uses some y- offsets in its pattern
                ...
                # Zoom in/out by changing cell_size and recreating the surface
            ...
            # Mouse painting: left = draw alive, right = erase
    ...
    def run(self):
        # Main loop: handle input, optionally step, then draw
        while self.handle_events():
            ...
            # When paused, cap at 60 to keep UI smooth; when running, use simulation speed
